...
// COMPILE-TIME CONFIG
// ============================================================================
template<int D, int PAD_OVERRIDE = -1>
struct KernelConfig {
    ...
    static constexpr int PAD = (PAD_OVERRIDE >= 0) ? PAD_OVERRIDE : ((8 - (D % 32) + 32) % 32);
...
// FORWARD KERNEL
// ============================================================================
template<int D, int PAD_OVERRIDE, bool IS_CAUSAL>
__global__ void __launch_bounds__(KernelConfig<D, PAD_OVERRIDE>::THREADS_PER_BLOCK, 2)
flash_attention_forward_kernel(
    ...
    using Config = KernelConfig<D, PAD_OVERRIDE>;
...
template<int D, int PAD, bool IS_CAUSAL>
void run_test(int B, int H, int M, int N, float softmax_scale) {
    using Config = KernelConfig<D, PAD>;
    printf("\n=== FA2 Forward Emulation ===\n");
    printf("D=%d, PAD=%d, STRIDE_Q/KV=%d, STRIDE_S=%d, STRIDE_O=%d\n", D, PAD, Config::Q_STRIDE, Config::S_STRIDE, Config::O_STRIDE);
    printf("BLOCK_M=%d, BLOCK_N=%d, WARPS=%d, SMEM=%.2f KB\n", Config::BLOCK_M, Config::BLOCK_N, Config::WARPS_PER_BLOCK, Config::TOTAL_SMEM / 1024.0f);
    printf("CAUSAL=%s, B=%d, H=%d, M=%d, N=%d\n", IS_CAUSAL ? "true" : "false", B, H, M, N);

    size_t q_size = B * H * M * D * sizeof(__half);
    size_t k_size = B * H * N * D * sizeof(__half);
    size_t v_size = B * H * N * D * sizeof(__half);
    size_t o_size = B * H * M * D * sizeof(__half);
    ...
    CHECK_CUDA(cudaMemset(d_K, 0, k_size));
    CHECK_CUDA(cudaMemset(d_V, 0, v_size));
    ...
    if (smem > MAX_SMEM_PER_SM) {
        fprintf(stderr, "ERROR: SMEM exceeds 96KB (%zu bytes)\n", smem);
        exit(EXIT_FAILURE);
    }

    auto kernel = flash_attention_forward_kernel<D, PAD, IS_CAUSAL>;
    CHECK_CUDA(cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem));

    kernel<<<grid, block, smem>>>(d_Q, d_K, d_V, d_Out, d_LSE, B, H, M, N, softmax_scale);
...
int main() {
    ...
    constexpr int B = 1, H = 1, M = 128, N = 128;

    run_test<TEST_D, TEST_PAD, TEST_CAUSAL>(B, H, M, N, SCALE);
