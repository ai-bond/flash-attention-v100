    ...
    const float softmax_scale) 
{
    ...
    for (int idx = tid; idx < (valid_q_rows * d_stride_uint4); idx += THREADS_PER_BLOCK) {
        ...
        st_uint4(sQ_base + (row * d_stride_uint4 + vec_col) * 16, q_val, row);
    }
    __syncthreads();
    ...
    for (int block = 0; block < num_n_tiles; ++block) {
        ...
        for (int idx = tid; idx < (valid_kv_rows * d_stride_uint4); idx += THREADS_PER_BLOCK) {
            ...
            st_uint4(sK_base + (row * d_stride_uint4 + vec_col) * 16, k_val, row);
        }
        __syncthreads();
        ...
        if (tid < valid_q_rows * THREADS_PER_ROW) {
            ...
            uint32_t sS_row = __cvta_generic_to_shared(sS + row * N_STRIDE);
            uint32_t sP_row = __cvta_generic_to_shared(sP + row * N_STRIDE);
            uint32_t sO_row = __cvta_generic_to_shared(sO + row * D_STRIDE);
            ...
            for (int j = 0; j < vecs_per_thread; ++j) {
                ...
                if (vc < vec_cols) {
                    float4 v4 = ld_float4(sS_row + vc * 16, row);
            ...
            for (int j = 0; j < vecs_per_thread; ++j) {
                int vc = t_in_row + j * THREADS_PER_ROW;
                if (vc < vec_cols) {
                    st_half2(sP_row + vc * 8,     half_buffer[h2_idx++], row);
                    st_half2(sP_row + vc * 8 + 4, half_buffer[h2_idx++], row);
                }
            }
            ...
            if (block > 0) {
                ...
                for (int ov = t_in_row; ov < o_vec_count; ov += THREADS_PER_ROW) {
                    float4 v = ld_float4(sO_row + ov * 16, row);
                    v.x *= exp_diff; v.y *= exp_diff; v.z *= exp_diff; v.w *= exp_diff;
                    st_float4(sO_row + ov * 16, v, row);
                }
            }
        }
        __syncthreads();
        ...
        for (int idx = tid; idx < (valid_kv_rows * d_stride_uint4); idx += THREADS_PER_BLOCK) {
            ...
            st_uint4(sV_base + (row * d_stride_uint4 + vec_col) * 16, v_val, row);
        }
        __syncthreads();
    ...
    uint32_t sO_base = __cvta_generic_to_shared(sO);
    for (int i = tid; i < (valid_q_rows * D) / 4; i += THREADS_PER_BLOCK) {
        int row = i / (D / 4);
        int col = (i % (D / 4)) * 4;
        float inv_sum = 1.0f / fmaxf(smem.row_sum[row], 1e-24f);
        uint32_t addr = sO_base + (row * D_STRIDE + col) * 4;

        float4 v4 = ld_float4(addr, row);
...
void run_test(int B, int H, int M, int N, float scale) {
    using Config = KernelConfig<D>;
    printf("\n=== FA2 Forward swizzled (D=%d, N_STRIDE=%d, SMEM=%.2f KB) ===\n", D, Config::N_STRIDE, Config::TOTAL_SMEM / 1024.0f);
