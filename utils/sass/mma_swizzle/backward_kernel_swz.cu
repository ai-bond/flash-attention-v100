    ...
    const float softmax_scale) 
{
    ...
    if (blockIdx.y == 0) {
        ...
        uint32_t sQ_base  = __cvta_generic_to_shared(sQ);
        uint32_t sdO_base = __cvta_generic_to_shared(sdO);
        ...
        for (int idx = tid; idx < valid_q_rows * D_UINT4; idx += THREADS_PER_BLOCK) {
            int row = idx / D_UINT4;
            int col = idx % D_UINT4;
            uint4 qv  = __ldg(&q_vec_g [row * D_UINT4 + col]);
            uint4 dv  = __ldg(&do_vec_g[row * D_UINT4 + col]);
            st_uint4(sQ_base  + (row * D_UINT4 + col) * 16, qv, row);
            st_uint4(sdO_base + (row * D_UINT4 + col) * 16, dv, row);
        }
        __syncthreads();
        ...
        if (tid < valid_q_rows * THREADS_PER_ROW) {
            ...
            for (int j = t_in_row; j < D; j += THREADS_PER_ROW) {
                ...
                float dv = ld_float(sdO_base + (row * D_STRIDE + j) * 4, row);
                thread_dot += ov * dv;
            }
        ...
        for (int block = 0; block < num_kv_tiles; ++block) {
            ...
            uint32_t sV_base = __cvta_generic_to_shared(sV);
            ...
            for (int idx = tid; idx < valid_kv_rows * D_UINT4; idx += THREADS_PER_BLOCK) {
                int row = idx / D_UINT4;
                int col = idx % D_UINT4;
                uint4 vv = __ldg(&v_vec_g[row * D_UINT4 + col]);
                st_uint4(sV_base + (row * D_UINT4 + col) * 16, vv, row);
            }
            __syncthreads();
            ...
            uint32_t sK_base = __cvta_generic_to_shared(sK);
            const uint4* k_vec_g = reinterpret_cast<const uint4*>(k_ptr + start_kv * D);
            for (int idx = tid; idx < valid_kv_rows * D_UINT4; idx += THREADS_PER_BLOCK) {
                int row = idx / D_UINT4;
                int col = idx % D_UINT4;
                uint4 kv = __ldg(&k_vec_g[row * D_UINT4 + col]);
                st_uint4(sK_base + (row * D_UINT4 + col) * 16, kv, row);
            }
            __syncthreads();
            ...
            if (tid < valid_q_rows * THREADS_PER_ROW) {
                ...
                uint32_t ss_base   = __cvta_generic_to_shared(sS   + row * N_STRIDE);
                uint32_t sdov_base = __cvta_generic_to_shared(sdOV + row * N_STRIDE);
                uint32_t sds_base  = __cvta_generic_to_shared(sdS  + row * N_STRIDE);
                for (int c = t_in_row; c < BLOCK_N; c += THREADS_PER_ROW) {
                    float s_val = (c < valid_kv_rows) ? ld_float(ss_base   + c * 4, row) : NEG_INF;
                    float dov   = (c < valid_kv_rows) ? ld_float(sdov_base + c * 4, row) : 0.0f;
                    float p = (s_val - lse_val < -80.0f) ? 0.0f : __expf(s_val - lse_val);
                    float ds = p * softmax_scale * (dov - row_dot_val);
                    st_half(sds_base + c * 2, __float2half_rn(ds), row);
                }
            }
            __syncthreads();
        ...
        uint32_t sdQ_base = __cvta_generic_to_shared(sdQ);
        for (int i = tid; i < valid_q_rows * D_UINT4; i += THREADS_PER_BLOCK) {
            int row = i / D_UINT4;
            int col = i % D_UINT4;
            uint32_t addr = sdQ_base + (row * D_STRIDE + col * 8) * 4;
            float4 v0 = ld_float4(addr,      row);
            float4 v1 = ld_float4(addr + 16, row);
            __half out[8];
        ...
        for (int block = 0; block < num_q_tiles; ++block) {
            ...
            if (tid < valid_q_rows * THREADS_PER_ROW) {
                ...
                float thread_dot = 0.0f;
                for (int j = t_in_row; j < D; j += THREADS_PER_ROW) {
                    float ov = __half2float(cur_o_ptr[row * D + j]);
                    float dv = ld_float(sdO_base + (row * D_STRIDE + j) * 4, row);
                    thread_dot += ov * dv;
                }
                for (int o = THREADS_PER_ROW / 2; o > 0; o >>= 1)
...

            {
                ...
                uint32_t ss_base   = __cvta_generic_to_shared(sS);
                uint32_t sdov_base = __cvta_generic_to_shared(sdOV);
                uint32_t sp_base   = __cvta_generic_to_shared(sP);
                uint32_t sds_base  = __cvta_generic_to_shared(sdS);
                for (int i = tid; i < total_elems; i += THREADS_PER_BLOCK) {
                    ...
                    if constexpr (IS_CAUSAL) {
                        if (global_n > global_m) {
                            st_half(sp_base  + (r * M_STRIDE + c) * 2, __float2half_rn(0.0f), r);
                            st_half(sds_base + (r * M_STRIDE + c) * 2, __float2half_rn(0.0f), r);
                            continue;
                        }
                    }

                    float s   = ld_float(ss_base   + (r * M_STRIDE + c) * 4, r);
                    float dov = ld_float(sdov_base + (r * M_STRIDE + c) * 4, r);
                    float lse_val = sLse[r];
                    float row_dot_val = sRowDot[r];
                    float p = (s - lse_val < -80.0f) ? 0.0f : __expf(s - lse_val);
                    float ds = p * softmax_scale * (dov - row_dot_val);
                    st_half(sp_base  + (r * M_STRIDE + c) * 2, __float2half_rn(p),  r);
                    st_half(sds_base + (r * M_STRIDE + c) * 2, __float2half_rn(ds), r);
                }
            }
            __syncthreads();
            ...
            {
                uint32_t sQ_base2 = __cvta_generic_to_shared(sQ);
                for (int idx = tid; idx < valid_q_rows * D_UINT4; idx += THREADS_PER_BLOCK) {
                    int row = idx / D_UINT4;
                    int col = idx % D_UINT4;
                    uint4 qv = __ldg(&q_vec_g[row * D_UINT4 + col]);
                    st_uint4(sQ_base2 + (row * D_UINT4 + col) * 16, qv, row);
                }
            }
            __syncthreads();
        ...
        uint32_t sdK_base = __cvta_generic_to_shared(sdK);
        uint32_t sdV_base = __cvta_generic_to_shared(sdV);
        for (int i = tid; i < valid_kv_rows * D_UINT4; i += THREADS_PER_BLOCK) {
            int row = i / D_UINT4;
            int col = i % D_UINT4;
            uint32_t addr_k = sdK_base + (row * D_STRIDE + col * 8) * 4;
            uint32_t addr_v = sdV_base + (row * D_STRIDE + col * 8) * 4;
            float4 k0 = ld_float4(addr_k,      row);
            float4 k1 = ld_float4(addr_k + 16, row);
            float4 v0 = ld_float4(addr_v,      row);
            float4 v1 = ld_float4(addr_v + 16, row);
            __half ok[8], ov[8];
            ok[0] = __float2half_rn(k0.x); ok[1] = __float2half_rn(k0.y);
            ok[2] = __float2half_rn(k0.z); ok[3] = __float2half_rn(k0.w);
            ok[4] = __float2half_rn(k1.x); ok[5] = __float2half_rn(k1.y);
            ok[6] = __float2half_rn(k1.z); ok[7] = __float2half_rn(k1.w);
            ov[0] = __float2half_rn(v0.x); ov[1] = __float2half_rn(v0.y);
            ov[2] = __float2half_rn(v0.z); ov[3] = __float2half_rn(v0.w);
            ov[4] = __float2half_rn(v1.x); ov[5] = __float2half_rn(v1.y);
            ov[6] = __float2half_rn(v1.z); ov[7] = __float2half_rn(v1.w);
