// Matrix B row_major register dump
#include <cuda_fp16.h>
#include <cstdint>
#include <cstdio>
...
__global__ void dump_b_row_regs(
    ...
    const uint32_t* src = reinterpret_cast<const uint32_t*>(b_frag.x);
    #pragma unroll
    for (int i = 0; i < 8; i++) {
        dst[i] = src[i];
    }
...
int main() {
    printf("Matrix B row_major dump\n");

    // B in ROW-MAJOR: B[i][j] = i*16 + j, stored at offset i*16 + j
    half h_B[256];
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
            h_B[i * 16 + j] = __float2half((float)(i * 16 + j));
        }
    }
    ...
    dump_b_row_regs<<<1, 32>>>(d_B, d_regs);
