...
// ======================================================================================
__device__ __forceinline__ void load_matrix_sw(fragment<matrix_a, 16, 16, 16, half, row_major>& frag, const half* __restrict__ smem_ptr, unsigned ldm) {
    unsigned smem_addr = __cvta_generic_to_shared(smem_ptr);
    asm volatile(
        "{\n\t"
...
// Swizzle:         ABSOLUTE XOR applied independently to each absolute address.
// ======================================================================================
__device__ __forceinline__ void load_matrix_sw(fragment<matrix_a, 16, 16, 16, half, col_major>& frag, const half* __restrict__ smem_ptr, unsigned ldm) {
    unsigned smem_addr = __cvta_generic_to_shared(smem_ptr);
    asm volatile(
        "{\n\t"
...
// Swizzle:         ABSOLUTE XOR applied independently to each absolute address.
// ======================================================================================
__device__ __forceinline__ void load_matrix_sw(fragment<matrix_b, 16, 16, 16, half, row_major>& frag, const half* __restrict__ smem_ptr, unsigned ldm) {
    unsigned smem_addr = __cvta_generic_to_shared(smem_ptr);
    asm volatile(
        "{\n\t"
...
// Swizzle:         ABSOLUTE XOR applied independently to each absolute address.
// ======================================================================================
__device__ __forceinline__ void load_matrix_sw(fragment<matrix_b, 16, 16, 16, half, col_major>& frag, const half* __restrict__ smem_ptr, unsigned ldm) {
    unsigned smem_addr = __cvta_generic_to_shared(smem_ptr);
    asm volatile(
        "{\n\t"
...
// Memory access:   ROW: Pairs row-contiguous. Stride=2*ldm*4B. Optimal: 4x v2.
//                  COL: Pairs col-contiguous. Elements strided by ldm. Optimal: 8x scalar.
// Swizzle:         ABSOLUTE POSITION-DEPENDENT XOR (bits 4-5 ^= bits 7-8).
//                  Applied to final address AFTER adding base pointer.
// ======================================================================================
__device__ __forceinline__ void load_matrix_sw(
    fragment<accumulator, 16, 16, 16, float>& frag,
    const float* __restrict__ smem_ptr,
    unsigned ldm,
    layout_t layout) 
{
    unsigned smem_addr = __cvta_generic_to_shared(smem_ptr);
    ...
    if (layout == mem_row_major) {
        asm volatile(
            ...
            // Absolute XOR swizzle applied independently to each address
            "shr.b32 sw, a0, 7; and.b32 sw, sw, 3; shl.b32 sw, sw, 4; xor.b32 a0, a0, sw;\n\t"
            "shr.b32 sw, a1, 7; and.b32 sw, sw, 3; shl.b32 sw, sw, 4; xor.b32 a1, a1, sw;\n\t"
            "shr.b32 sw, a2, 7; and.b32 sw, sw, 3; shl.b32 sw, sw, 4; xor.b32 a2, a2, sw;\n\t"
            "shr.b32 sw, a3, 7; and.b32 sw, sw, 3; shl.b32 sw, sw, 4; xor.b32 a3, a3, sw;\n\t"
...
//                  Applied to final address AFTER adding base pointer.
// ======================================================================================
__device__ __forceinline__ void store_matrix_sw(
