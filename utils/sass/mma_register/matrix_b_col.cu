...
int main() {
    printf("Matrix B col_major dump\n");

    // B in COL-MAJOR: B[i][j] = i*16 + j, stored at offset i + j*16
    half h_B[256];
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
            h_B[i + j * 16] = __float2half((float)(i * 16 + j));
        }
    }
